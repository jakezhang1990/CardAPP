...
import com.cardlan.carddemoapp.data.KeyErrorException;
import com.cardlan.carddemoapp.data.TerminalConsumeDataForSystem;
import com.cardlan.carddemoapp.thread.DefaultBaseNonUIThread;
import com.cardlan.carddemoapp.util.CardReadWriteUtil;
import com.cardlan.carddemoapp.util.CardlanLog;
import com.cardlan.carddemoapp.util.CpuCardSecretKeyHelper;
import com.cardlan.carddemoapp.util.SharedPreferencesHelper;
...
public class DemoActivity
        extends Activity implements View.OnClickListener, View.OnLongClickListener, CardLanSpiHelper.SpiCallBackLinstener {

    private static final int S_Reset_buffer_size = 16;
...
//                String key_write = "0A1F559F4A81";
                String key_write = "0ACFD6AE4A81";
                ...
                serialTest.setCallBack(new CardLanSerialHelper.FileCallBack() {
                    ...
                    public void getResult(byte[] buffer, int size) {
                        ...
                        runOnUiThread(new Runnable() {
                            @Override
                            public void run() {
                                if (ByteUtil.notNull(realQrCode)) {
                                    //CardLanDevUtil.openProc();
                                    mTv_qc_result.setText(": " + realQrCode);
                                    terminal.callProc();
...
//	        String result = streamRead(fis);
    ...
    @Override
    public void receivePasmData(String code, byte[] respondBuff) {
    ...
    @Override
    public void receiveSpiData(String code, byte[] respondBuff) {
        if (ByteUtil.notNull(respondBuff)) {
            Log.d("respondBuff", "respondBuff = " + ByteUtil.byteArrayToHexString(respondBuff));
        }
    }
